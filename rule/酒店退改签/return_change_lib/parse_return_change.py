...
import datelib
import math
import elong_display
import agoda_display

source_rule = {}
source_rule['booking'] = 'others_info'
source_rule['ctrip'] = 'return_rule'
source_rule['expedia'] = 'return_rule'
source_rule['hotels'] = 'return_rule'
source_rule['venere'] = 'return_rule'
source_rule['accor'] = 'return_rule'
source_rule['agoda'] = 'return_rule'
source_rule['ctrip'] = 'return_rule'
source_rule['ebookers'] = 'return_rule'
source_rule['elong'] = 'return_rule'
source_rule['hilton'] = 'return_rule'
source_rule['hotelclub'] = 'return_rule'
source_rule['hoteltravel'] = 'return_rule'
source_rule['hrs'] = 'return_rule'
source_rule['ihg'] = 'return_rule'
source_rule['marriot'] = 'return_rule'
source_rule['orbitz'] = 'return_rule'
source_rule['travelocity'] = 'return_rule'


def ERROR(tmp_str):
	sys.stderr.write(tmp_str + '\n')

def no_return_no_show(display_list,date_price_map):
	...
	if find_flag == True:
		...
		for list_t in [display_list]:
			...
			if find_idx != -1:
				list_t.pop(find_idx)
		if date_price_map.has_key(no_show_key):
			date_price_map.pop(no_show_key)
...
def returnChange2Sql(source,resultList,date_price_list):
	...
	date_price_map = {}
	if len(resultList) != 3:
		sys.stderr.write('Fatal error result_list num not enough\n')
		return sqlList,displayList,date_price_map

	cancleLists = resultList[0]
	freeFlag = False
	date_price_default = ['',-1.0]
	displayMap = {}
	keyList = []
	try:
	...
	for cancleList in cancleLists:
		...
		if len(date_price) > 1 and isinstance(date_price[1],float):
			date_price[1] = math.ceil(date_price[1])

		displayInfo = parse2display.displayReturnChange(mapKey,source,cancleList,date_price)
		...
		if displayMap.has_key(mapKey) == False:
			...
			date_price_map[mapKey] = []
		displayMap[mapKey].append(displayInfo)
		desc_str = displayInfo
		time_desc = date_price[0]
		value_float = float(date_price[1])
		tmp_map = {'desc':desc_str,'timeDesc':time_desc,'val':value_float}

		#return_date_price_map[mapKey].append(date_price)
		date_price_map[mapKey].append(tmp_map)
		idx += 1

	if len(keyList) == 1 and 'no_return_change' == keyList[0]:
		displayMap['no_return_change'][0] = displayMap['no_return_change'][0].replace('超出时限','')
		date_price_map['no_return_change'][0]['desc'] = date_price_map['no_return_change'][0]['desc'].replace('超出时限','')
...
def parseReturnChange(source,return_rule,change_rule,others_info,short_md5,\
	...
	date_price_map = {}
	status_flag = True
	if source == 'booking' and (others_info.lower().replace('null','').strip() != ''):
		parseFlag = True
		infoStr = others_info
		status_flag,resList = booking_func.parseOthersInfo(infoStr)
		if status_flag == True:
...
#		date_price_list = elong_func.get_date_price(resList,check_in_date,live_day,price_all,currency,tax,currency_map)
		#display_res_list(resList)
		#display_price(date_price_list)
	elif (source == 'hotels' or source == 'venere') and (return_rule.lower().replace('null','').strip() != ''):
		parseFlag = True
		infoStr = return_rule
		status_flag,resList = hotels_func.hotelsCancle(infoStr)
		if status_flag == True:
			date_price_list = hotels_func.get_date_price(resList,check_in_date,live_day,price_all,currency,tax,currency_map)
		#display_res_list(resList)
		#display_price(date_price_list)

	elif source == 'ctrip' and (return_rule.lower().replace('null','').strip() != ''):
		parseFlag = True
		infoStr = return_rule
		status_flag,resList = ctrip_func.parse_ctrip_return(infoStr)

	elif source == 'hoteltravel' and (return_rule.lower().replace('null','').strip() != ''):
		parseFlag = True
		infoStr = return_rule
		status_flag,resList = hoteltravel_func.process_info(infoStr)
		if status_flag:
			date_price_list = hoteltravel_func.get_date_price(resList,check_in_date,live_day,price_all,currency,tax,currency_map)

	elif source == 'expedia' and (return_rule.lower().replace('null','').strip() != ''):
		parseFlag = True
		infoStr = change_rule
		status_flag,resList = expedia_func.process_info(infoStr)

	if status_flag == False:
		print 'return_change_err_tag\thotel\t%s\t%s' % (source,infoStr)
	elif parseFlag:
		print 'return_change_tag_ori\thotel\t%s' % source
		returnChangeTable(source,resList,short_md5,relationList,return_change_list)
		sqlList,displayList,date_price_map = returnChange2Sql(source,resList,date_price_list)
...
#		ERROR('')
	...
	no_return_no_show(displayList,date_price_map)

	if source == 'elong' and return_rule != 'NULL' and (return_rule.lower().replace('null','').replace(' ','').strip() != ''):
		if currency_map.has_key(currency):
			if tax < 0:
				tax = 0
			new_price = (price_all + tax) * currency_map[currency]
			sqlList,displayList,date_price_map = elong_display.parse_all(return_rule,live_day,new_price)
		else:
			sqlList,displayList,date_price_map = elong_display.parse_all(return_rule,live_day,-1.0)
	if source == 'agoda' and return_rule != 'NULL' and (return_rule.lower().replace('null','').replace(' ','').strip() != ''):
		if currency_map.has_key(currency):
			if tax < 0:
				tax = 0
			new_price = (price_all + tax) * currency_map[currency]
			sqlList,displayList,date_price_map = agoda_display.parse_all(return_rule,live_day,new_price,check_in_date)
		else:
			sqlList,displayList,date_price_map = agoda_display.parse_all(return_rule,live_day,-1.0,check_in_date)
